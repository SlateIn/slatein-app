...
export class AlertReminderService {
  ...
  minDate = `${this.todayDate.getFullYear()}-${String(
    this.todayDate.getMonth() + 1
  ).padStart(2, "0")}-${String(this.todayDate.getDate()).padStart(2, "0")}`;
  // tslint:disable-next-line: max-line-length
  maxDate = `${this.todayDate.getFullYear() + 15}-${String(
    this.todayDate.getMonth() + 1
  ).padStart(2, "0")}-${String(this.todayDate.getDate()).padStart(2, "0")}`;
    ...
    const alert = await this.alertController.create({
      ...
      buttons: [
        ...
        },
        {
          text: "Ok",
          handler: (
            repeat: "year" | "month" | "two-weeks" | "week" | "day"
          ) => {
  ...
  async onSubmit(
    value: Reminder,
    repeat: "year" | "month" | "two-weeks" | "week" | "day" | "never" = "never"
  ) {
    const startTimePeriod = new Date(
